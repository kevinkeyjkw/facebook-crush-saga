...
public class CrushSagaPanel extends JPanel{
    ...
    Font levelDialogTitleFont = new Font("Arial",Font.BOLD,40);;
    ...
    public void paintComponent(Graphics g)
    {
        try
        {
            ...
            if(data.getMallet()){
                ...
                imgPath = props.getProperty(CrushSagaPropertyType.IMG_PATH);
                img = game.loadImage(imgPath + props.getProperty(CrushSagaPropertyType.MALLET_IMAGE_NAME));
                c = tk.createCustomCursor(img, new Point(this.getX(),this.getY()), MALLET_CURSOR);
                this.setCursor(c);
            }else{
                this.setCursor(null);
            }
    ...
    private void renderTiles(Graphics g)
    {
        ...
        CrushSagaTile[][] jellyGrid = data.getJellyGrid();
        for(CrushSagaTile tileArray[]:jellyGrid){
            for(CrushSagaTile tile: tileArray){
                renderTile(g,tile);
            }
        }
    ...
    public void renderStats(Graphics g)
    {
        if(((CrushSagaMiniGame)game).isCurrentScreenState(SAGA_SCREEN_STATE) && ((CrushSagaMiniGame)game).getGUIDialogs().get(LEVEL_DIALOG_TYPE).getState().equals(VISIBLE_STATE))
        {
            ...
            g.setFont(levelDialogTitleFont);
            g.setColor(Color.WHITE);
            String currentLevel = data.getLevelName();
            g.drawString(currentLevel, LEVEL_DIALOG_TITLE_X, LEVEL_DIALOG_TITLE_Y);
            g.setFont(statsFont);
