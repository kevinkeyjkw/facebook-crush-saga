...
public class CrushSagaMiniGame extends MiniGame{
    ...
    public void initGUIControls()
    {
        ...
        x = (gameWidth - totalWidth)/2.0f;
            ...
            s = new Sprite(sT,PLAY_GAME_BUTTON_X,PLAY_GAME_BUTTON_Y,0,0,VISIBLE_STATE);
            ...
            sT.addState(MOUSE_OVER_STATE,img);
            s = new Sprite(sT,QUIT_GAME_BUTTON_X,QUIT_GAME_BUTTON_Y,0,0,VISIBLE_STATE);
            ...
            sT.addState(MOUSE_OVER_STATE,img);
            s = new Sprite(sT,RESET_GAME_BUTTON_X,RESET_GAME_BUTTON_Y,0,0,VISIBLE_STATE);
            guiButtons.put(RESET_GAME_BUTTON_TYPE,s);
            
            //INITIATE GAME SCREEN GUI
            initGameScreenGUI();
            //INITIATE SAGA BUTTONS
            initSagaButtons();
          
    }
    public void initGameScreenGUI(){
            PropertiesManager props = PropertiesManager.getPropertiesManager();
            String imgPath = props.getProperty(CrushSagaPropertyType.IMG_PATH);
            BufferedImage img;
            SpriteType sT;
        ...
        //initiate win dialog
            ...
            //INITIATE PROGRESS BAR AND ITS TICKS
            ArrayList<String> progressBarImageNames = props.getPropertyOptionsList(CrushSagaPropertyType.PROGRESS_BAR_IMAGE_NAMES);
            sT = new SpriteType(PROGRESS_BAR_TYPE);
            img = loadImage(imgPath + progressBarImageNames.get(0));
            sT.addState(VISIBLE_STATE, img);
            sT.addState(PROGRESS_ONE_STATE,img);
            img = loadImage(imgPath + progressBarImageNames.get(1));
            sT.addState(PROGRESS_TWO_STATE,img);
            img = loadImage(imgPath + progressBarImageNames.get(2));
            sT.addState(PROGRESS_THREE_STATE,img);
            img = loadImage(imgPath + progressBarImageNames.get(3));
            sT.addState(PROGRESS_FOUR_STATE,img);
            img = loadImage(imgPath + progressBarImageNames.get(4));
            sT.addState(PROGRESS_FIVE_STATE,img);
            img = loadImage(imgPath + progressBarImageNames.get(5));
            sT.addState(PROGRESS_SIX_STATE,img);
            img = loadImage(imgPath + progressBarImageNames.get(6));
            sT.addState(PROGRESS_SEVEN_STATE,img);
            img = loadImage(imgPath + progressBarImageNames.get(7));
            sT.addState(PROGRESS_EIGHT_STATE,img);
            img = loadImage(imgPath + progressBarImageNames.get(8));
            sT.addState(PROGRESS_NINE_STATE,img);
            img = loadImage(imgPath + progressBarImageNames.get(9));
            sT.addState(PROGRESS_TEN_STATE,img);
            s = new Sprite(sT,PROGRESS_BAR_X,PROGRESS_BAR_Y,0,0,INVISIBLE_STATE);
            guiDecor.put(PROGRESS_BAR_TYPE,s);
    }
    ...
    public void initLevelButtons()
    {
        ...
        {
            if(j < 10)
                LEVEL_POSITIONS_Y[j] = 500;
            else if(j < 20)
                LEVEL_POSITIONS_Y[j] = 370;
            else if(j < 30)
                LEVEL_POSITIONS_Y[j] = 240;
            else if(j < 40)
                LEVEL_POSITIONS_Y[j] = 110;
            else if(j < 50)
                LEVEL_POSITIONS_Y[j] = -20;
            else if(j < 60)
                LEVEL_POSITIONS_Y[j] = -150;
            else if(j < 70)
                LEVEL_POSITIONS_Y[j] = -280;
            else if(j < 80)
                LEVEL_POSITIONS_Y[j] = -410;
            else if(j < 90)
                LEVEL_POSITIONS_Y[j] = -540;
            else
                LEVEL_POSITIONS_Y[j] = -670;
        }       
            for(int i = 0;i < levels.size();i++){
